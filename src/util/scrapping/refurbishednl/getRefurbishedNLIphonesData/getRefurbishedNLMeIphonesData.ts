...
async function getIphoneProductsDataFromSearchResults (page: playwright.Page, iphone: IphoneGroup, link: string) {
    ...
    const products: IphonePublicationData [] = await page.$$eval('.productSingleForm', results => {
        return results.map((el) => {
            ...
            const publicationTitle = `${el.querySelector('.productTitle > a')?.textContent} - renewed` //I


            el.querySelector('.productPrice > span')?.remove() //We remove the span element so we dont have to delete its content manually when getting .productPrice
            //text content
            let price = el.querySelector('.productPrice')?.textContent?.replace(/,[0-9]*/, '').match(/[0-9]{1,6}/g)?.join('')  //Price
        ...
        console.log('\x1b[31m%s\x1b[0m', '!---------------------------------------!')
        console.log('\x1b[31m%s\x1b[0m', `Ha ocurrido un error al momento de buscar informacion sobre el Iphone ${iphone.name} en RefurbishedNL. El error es: `)
    ...
    products.forEach((obtainedProductData) => {
        ...
        if (obtainedProductData.publicationTitle.toLowerCase().includes('iphone 15 pro max')) {
            console.log(obtainedProductData.price)
        }
...
export async function getRefurbishedNLIphonesData () {  
      
    const browser = await chromium.launch({headless: true})
    const page = await browser.newPage()

    const pagesToVisit = ["2", "3", "4"]

    for (const iphone of iphones.groups) {
        const link = `${iphone.scrapperLinks.RefurbishedNL}`
        console.log('\x1b[34m%s\x1b[0m', '---------------------------------------')
        console.log('\x1b[34m%s\x1b[0m', `Recogiendo datos del ${iphone.name} en RefurbishedNL.`)
        console.log('\x1b[34m%s\x1b[0m', `Pagina: 1`)
        console.log('\x1b[34m%s\x1b[0m', "Link de RefurbMe: " + link)
        console.log('\x1b[34m%s\x1b[0m', '---------------------------------------')
        await page.goto(link, {timeout: 10000}).catch(error => console.log('\x1b[31m%s\x1b[0m', 'Error al intentar navegar a la pagina, posiblemente por conexion lenta'))
        await getIphoneProductsDataFromSearchResults(page, iphone, link).catch(error => {
            console.log('\x1b[31m%s\x1b[0m','---------------------------------------')
        ...
        await new Promise(resolve => setTimeout(resolve, Math.floor(Math.random() * 1000 + 5000)))
    ...
    for (const iphone of iphones.groups) {
        ...
        for (const numberOfPage of pagesToVisit) {
            const link = `${iphone.scrapperLinks.RefurbishedNL}?page=${numberOfPage}`
            console.log('\x1b[37m%s\x1b[0m', '---------------------------------------')
            console.log('\x1b[37m%s\x1b[0m',`Recogiendo datos del ${iphone.name} en RefurbishedNL.`)
            console.log('\x1b[37m%s\x1b[0m',`Pagina: ${numberOfPage}`)
            console.log('\x1b[37m%s\x1b[0m',"Link de RefurbishedNL: " + link)
            console.log('\x1b[37m%s\x1b[0m','---------------------------------------')
